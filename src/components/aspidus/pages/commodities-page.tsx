...
export default function CommoditiesPage({ initialSector }: { initialSector?: string }) {
  ...
  return (
    <div className="pt-28 sm:pt-32 page-enter">
      {/* Page hero */}
      <section className="relative pb-12 sm:pb-16 border-b border-[rgba(201,165,92,0.12)]">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-end">
            <div className="lg:col-span-8">
              ...
              <h1
                className="font-serif text-4xl sm:text-5xl lg:text-6xl text-[var(--parchment)] leading-[1.05] tracking-tight"
                data-reveal
            ...
            <div className="lg:col-span-4" data-reveal data-reveal-delay="0.2">
              <p className="text-[var(--parchment-dim)] leading-relaxed">{t("catalog.desc")}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Filters + search */}
      <section className="sticky top-[68px] z-30 bg-[#060d18]/90 backdrop-blur-xl border-b border-[rgba(201,165,92,0.12)] py-4">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center justify-between">
            ...
            <div className="relative lg:w-72 flex-shrink-0">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--parchment-dim)]" />
              <input
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder={t("catalog.searchPlaceholder")}
                className="w-full pl-10 pr-9 py-2.5 bg-[rgba(255,255,255,0.04)] border border-[rgba(201,165,92,0.18)] rounded-sm text-sm text-[var(--parchment)] placeholder:text-[var(--parchment-dim)] focus:outline-none focus:border-[var(--primary)] transition-colors"
              />
              {query && (
                <button
                  onClick={() => setQuery("")}
                  className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[var(--parchment-dim)] hover:text-[var(--parchment)]"
                >
                  <X className="h-4 w-4" />
                </button>
              )}
        ...
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <div className="flex items-center justify-between mb-8">
            <div className="text-sm text-[var(--parchment-dim)]">
              ...
              {t("catalog.results")}
              {activeSector !== "all" && (
                <span className="ml-2 text-[var(--parchment-dim)]">— {sectorName(activeSector)}</span>
              )}
            </div>
          </div>
          ...
          {filtered.length === 0 ? (
            <div className="py-24 text-center">
              <p className="text-[var(--parchment-dim)] text-lg">{t("catalog.noResults")}</p>
            </div>
          ) : (
            <motion.div
              layout
                ...
                {filtered.map((p, i) => {
                  ...
                  return (
                    ...
                    >
                      <RLink
                        to={`/product/${p.slug}`}
                        className="group block h-full overflow-hidden rounded-sm border border-[rgba(201,165,92,0.12)] hover:border-[var(--primary)]/45 bg-[#0b1626] transition-colors duration-500"
                      >
                        <div className="relative aspect-[5/4] overflow-hidden">
                          ...
                          {sector && (
                            <div className="absolute top-3 left-3 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-sm bg-[#060d18]/80 backdrop-blur-sm border border-[rgba(201,165,92,0.25)] text-[0.62rem] font-semibold tracking-wider uppercase text-[var(--primary)]">
                              <sector.icon className="h-3 w-3" />
                              {sector.name}
                            </div>
                          )}
                          <ArrowUpRight className="absolute top-3 right-3 h-5 w-5 text-white/0 group-hover:text-white transition-all duration-500 -rotate-45 group-hover:rotate-0" />
                        </div>

                        <div className="p-5">
                          <h3 className="font-serif text-lg text-[var(--parchment)] leading-snug group-hover:text-[var(--primary)] transition-colors">
                            {p.name}
                          </h3>
                          <p className="mt-2 text-sm text-[var(--parchment-dim)] leading-relaxed line-clamp-2">
                            {p.shortDescription}
                          </p>
                          <div className="mt-4 pt-4 border-t border-[rgba(201,165,92,0.1)] flex items-center justify-between">
                            <span className="text-xs tracking-wider uppercase text-[var(--parchment-dim)]">
